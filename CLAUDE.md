...
- **Exportación — nada de pioneros**: **ninguna cifra que se exporte a imagen puede depender de `isPioneer()` mientras siga siendo local.** La stat de primeras llegadas salió del export el 27 jul 2026 por eso: `isPioneer()` solo compara contra las entradas del propio usuario y contra una lista hardcodeada de 33 destinos (`KNOWN_DESTINATIONS`), no consulta Supabase, así que el número puede ser sencillamente falso — y una lámina de export se publica en redes, donde no hay forma de corregirlo. No vuelve hasta que `isPioneer()` sea una consulta real.
...
- **Logros — tipografía de títulos**: DECIDIDO (25 jul 2026): los títulos de logro van en **Inter**, alineados con las rutas de Viajes (`.entry-route`), NO con el serif del Diario. Este píxel ya cambió de bando dos veces (`27fe33e` serif → `7ea4cdc` Inter); no reabrirlo sin decisión explícita de Paula. La sensación de "más grande que el diario" es la altura de x de Inter 500 frente a Instrument Serif 400 a igual cuerpo (0.92rem), y es deliberada.
